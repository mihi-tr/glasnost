...
rate,mode,test.test AS stest FROM result,test WHERE test_id=test.id GROUP
BY test.client_id,mode,test.port,test.test) summary ON summary.client_id=id
WHERE id NOT IN 
(SELECT id FROM client_results) GROUP BY client.id,summary.stest;""")
...
client_results.cc,client_results.test,count(id) as total,shaped.shaped from
...
as total, sum(shaped) as shaped from asn inner join (select
client_results.asn, client_results.test, count(id) as total,
sum(shaped.shaped) as shaped from client_results left outer join (select
...
group by client_results.asn,client_results.test) as r on r.asn=asn.asn
group by asn.owner,r.test;"""%last_year)
cur.execute("update provider_results set shaped=0 where shaped is Null");
